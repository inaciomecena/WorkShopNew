   ...
   public class aproductspercategory : GXWebProcedure
   {
      ...
      void executePrivate( )
      {
         ...
         try
         {
            ...
            getPrinter().GxDrawText("Catálogo", 393, Gx_line+40, 643, Gx_line+88, 0, 0, 0, 0) ;
            getPrinter().GxDrawLine(13, Gx_line+121, 828, Gx_line+121, 2, 0, 0, 0, 0) ;
            getPrinter().GxDrawBitMap(context.GetImagePath( "82a664b3-3df3-4cda-b374-f8e42bf48c8f", "", context.GetTheme( )), 33, Gx_line+27, 273, Gx_line+107) ;
            Gx_OldLine = Gx_line;
            ...
            while ( (pr_default.getStatus(0) != 101) )
            {
               ...
               while ( (pr_default.getStatus(0) != 101) && ( StringUtil.StrCmp(P000H2_A7CategoryName[0], A7CategoryName) == 0 ) )
               {
                  ...
                  getPrinter().GxDrawText(StringUtil.LTrim( context.localUtil.Format( A27ProductPrice, "R$ ZZZZ9.99")), 367, Gx_line+13, 471, Gx_line+35, 2+256, 0, 0, 0) ;
                  sImgUrl = (String.IsNullOrEmpty(StringUtil.RTrim( A28ProductPhoto)) ? A40000ProductPhoto_GXI : A28ProductPhoto);
                  getPrinter().GxDrawBitMap(sImgUrl, 27, Gx_line+13, 127, Gx_line+63) ;
                  getPrinter().GxDrawText(StringUtil.RTrim( context.localUtil.Format( A13ProductName, "")), 187, Gx_line+13, 334, Gx_line+35, 0+256, 0, 0, 0) ;
                  getPrinter().GxDrawText(StringUtil.RTrim( context.localUtil.Format( A18SellerName, "")), 487, Gx_line+13, 634, Gx_line+35, 0+256, 0, 0, 0) ;
                  sImgUrl = (String.IsNullOrEmpty(StringUtil.RTrim( A19SellerPhoto)) ? A40001SellerPhoto_GXI : A19SellerPhoto);
                  getPrinter().GxDrawBitMap(sImgUrl, 687, Gx_line+13, 787, Gx_line+63) ;
                  Gx_OldLine = Gx_line;
