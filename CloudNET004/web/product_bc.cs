...
namespace GeneXus.Programs {
   public class product_bc : GxSilentTrn, IGxSilentTrn
   {
      ...
      protected void CheckExtendedTable056( )
      {
         ...
         {
            GX_msglist.addItem(context.GetMessage( "GXM_1004", new   object[]  {"Nome do Produto"}), 1, "");
            AnyError = 1;
         }
         pr_default.close(7);
         if ( String.IsNullOrEmpty(StringUtil.RTrim( A13ProductName)) )
         {
            GX_msglist.addItem("O Nome não pode estar vazio", 1, "");
            AnyError = 1;
         }
         if ( (Convert.ToDecimal(0)==A27ProductPrice) )
         {
            GX_msglist.addItem("Preço não pode estar vazio", 1, "");
            AnyError = 1;
         }
